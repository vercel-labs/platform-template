...
async function testSnapshotCreation(
  vcpus: number,
): Promise<{ createTime: number; sandbox: Sandbox }> {
...
async function testDevServerStartup(
  sandbox: Sandbox,
): Promise<{ cmdTime: number; readyTime: number }> {
...
async function testCommandLatency(
  sandbox: Sandbox,
): Promise<{ simple: number; complex: number }> {
...
async function testFileOperations(
  sandbox: Sandbox,
): Promise<{ read: number; write: number; list: number }> {
  ...
  await sandbox.writeFiles([
    {
      path: "/vercel/sandbox/test-file.txt",
      content: Buffer.from("hello world " + Date.now()),
    },
  ]);
...
async function runBenchmarks(): Promise<TestResults> {
  ...
  try {
    ...
    for (const vcpus of [2, 4]) {
      ...
      results.results[`snapshot-create-${vcpus}vcpu`] = stats(
        `Snapshot Create (${vcpus} vCPU)`,
        times,
      );
    }
    ...
    if (!SKIP_COLD_BOOT) {
      ...
      results.results["cold-boot-2vcpu"] = stats("Cold Boot (2 vCPU)", [
        createTime,
      ]);
    }
    ...
    results.results["cmd-complex"] = stats(
      "Complex Command (ls)",
      complexTimes,
    );
...
function printSummary(results: TestResults) {
  ...
  if (e2e) {
    console.log(
      `• Time to responsive agent: ~${e2e.avg}ms (${(e2e.avg / 1000).toFixed(1)}s)`,
    );
  }

  if (devReady) {
    console.log(
      `• Dev server startup: ~${devReady.avg}ms after sandbox creation`,
    );
  }
