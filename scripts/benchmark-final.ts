...
async function benchmark(agentId: string) {
  ...
  console.log(`3. bunx create-next-app: ${times["create_next_app"]}ms (exit: ${createApp.exitCode})`);
  ...
  const [agentResult, devTime] = await Promise.all([agentPromise, devServerReady]);
  ...
  // Verify agent works
  const verifyCmd = agentId === "claude" 
  ...
  console.log(`   - Agent binary: ${(await verify.stdout()).trim() || "NOT FOUND"}`);
