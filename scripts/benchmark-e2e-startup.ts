...
const SNAPSHOTS = {
  ...
  current: process.env.NEXTJS_SNAPSHOT_ID || "snap_SEwZgtRKfWMorgL58D1BcXdVlAZE",
};
...
async function benchmarkSnapshot(name: string, snapshotId: string): Promise<Result> {
  console.log(`\n⏱️  Testing: ${name}`);
  ...
  try {
    ...
    for (let i = 0; i < 240; i++) { // 60 seconds max
      try {
...
async function main() {
  ...
  console.log("RESULTS");
  console.log("=".repeat(70));
  console.log("\n" + "Name".padEnd(20) + "Create".padEnd(10) + "Dev Ready".padEnd(12) + "Total".padEnd(10) + "Cmd After");
  console.log("-".repeat(70));
  ...
  for (const [name, runs] of Object.entries(groups)) {
    const avgCreate = Math.round(runs.reduce((a, r) => a + r.createTime, 0) / runs.length);
    const avgDevReady = Math.round(runs.reduce((a, r) => a + r.devServerReady, 0) / runs.length);
    const avgTotal = Math.round(runs.reduce((a, r) => a + r.totalToReady, 0) / runs.length);
    const avgCmd = Math.round(runs.reduce((a, r) => a + r.commandAfterReady, 0) / runs.length);
    
    console.log(`${name}: create=${avgCreate}ms, dev=${avgDevReady}ms, total=${avgTotal}ms, cmd=${avgCmd}ms`);
  }
